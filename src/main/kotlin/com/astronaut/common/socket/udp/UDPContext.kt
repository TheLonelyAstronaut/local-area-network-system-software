 ...
 * @param continuation [CancellableContinuation] of [UDPSendContext] - continuation that completes when send succeeds,
 *  completes exceptionally when socket closed before send completes, and can be canceled (that will cancel sending)
 */
abstract class UDPSendContext(
    val threadId: UUID,
    val packet: QueuedDatagramPacket,
    val repairBlockSizeBytes: Int,
    val continuation: CancellableContinuation<UDPSendContext>
) {
    var blockId: Int = 0
 ...
 */
class EncodingUDPSendContext(
    threadId: UUID,
    packet: QueuedDatagramPacket,
    repairBlockSizeBytes: Int,
    cancellableContinuation: CancellableContinuation<UDPSendContext>
) : UDPSendContext(threadId, packet, repairBlockSizeBytes, cancellableContinuation) {
 ...
 */
abstract class UDPReceiveContext(val threadId: UUID) {
 ...
 */
class DecodingUDPReceiveContext(threadId: UUID, messageSizeBytes: Int, repairBlockSizeBytes: Int) :
    UDPReceiveContext(threadId) {
 ...
 */
class DummyUDPReceiveContext(threadId: UUID) : UDPReceiveContext(threadId) {
 ...
 */
class UDPContextManager {
    private val sendContexts = mutableMapOf<UUID, UDPSendContext>()
    private val receiveContexts = mutableMapOf<UUID, Pair<Long, UDPReceiveContext>>()
    ...
    fun createOrGetSendContext(
        ...
        sendContexts.getOrPut(threadId) {
            if (packet.data.limit() > repairBlockSizeBytes)
                EncodingUDPSendContext(threadId, packet, repairBlockSizeBytes, cancellableContinuation)
            else
                DummyUDPSendContext(threadId, packet, repairBlockSizeBytes, cancellableContinuation)
        }

    fun destroySendContext(threadId: UUID): UDPSendContext? {
    ...
    fun createOrGetReceiveContext(threadId: UUID, messageSizeBytes: Int, repairBlockSizeBytes: Int) =
        receiveContexts.getOrPut(threadId) {
            if (messageSizeBytes == repairBlockSizeBytes)
                System.currentTimeMillis() to DummyUDPReceiveContext(threadId)
            else
                System.currentTimeMillis() to DecodingUDPReceiveContext(
                    threadId,
                    messageSizeBytes,
                    repairBlockSizeBytes
                )
    ...
    fun destroyReceiveContext(threadId: UUID): UDPReceiveContext? {
